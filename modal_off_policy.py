...
app = modal.App("cs224r-hw2-off-policy")
...
image = (
    ...
    .add_local_python_source("off_policy", "train_off_policy", "mw", "utils", "logger", "replay_buffer", "video")
...
def train():
    ...
    subprocess.run(["find", "/root", "-name", "off_policy*"], check=True)
    subprocess.run(["python", "-c", "import off_policy; print(off_policy.__file__)"], cwd="/root")
    
    import torch
    print("torch version", torch.__version__)
    # Patch the config in-place from its already-mounted location
    config_path = "/root/cfgs/off_policy_config.yaml"
    ...
    if "hydra" in cfg:
        ...
        cfg["hydra"]["run"] = {"dir": "/output/off_policy_run"}
